...
def setup_logging():
    ...
    log_filename = f"logs/training_complex_fast_{timestamp}.log"
...
class DQN(nn.Module):
    ...
    def __init__(self, actions):
        ...
        # 保持原始全连接层复杂度
        self.fc1 = nn.Linear(64, 512)  # 保持原始神经元数
        self.fc2 = nn.Linear(512, actions)
        
        # 添加额外的复杂层 (可选)
        self.dropout = nn.Dropout(0.1)  # 添加dropout防止过拟合
        self.batch_norm1 = nn.BatchNorm1d(512)  # 添加批归一化
...
def train_network():
    ...
    logging.info("🚀 开始Flappy Bird AI复杂网络快速训练")
    logging.info("=" * 60)
    ...
    # 尝试加载预训练模型
    model_path = "saved_networks/bird-dqn-pytorch-complex-fast.pth"
    ...
    while "flappy bird" != "angry bird":
        ...
        if frame_count % FRAME_PER_ACTION == 0:
            ...
            if agent.step % 5000 == 0:
                ...
                agent.save_model(f"saved_networks/{GAME}-dqn-pytorch-complex-fast-{agent.step}.pth")
