...
**阶段 1、阶段 2：完成，验收都过了。** 左右分屏的对战页面（`web/index.html`）
已经能跑：玩家一边、AI 一边、同一个 seed、AI 全程不停。
下一步是阶段 3 剩下的**讲解与移动端**部分。
...
- [ ] **阶段 3　同台竞技 + 讲解**
  ...
  - [ ] 移动端布局（现在是 `grid-template-columns: 1fr 1fr` + `clamp()` 字号，
        窄屏能用但没在真机上验过；页面文案已经是英文）
- [ ] **阶段 4　发布**
  - [ ] Cloudflare Pages 配置
  - [ ] OG 卡片图
  - [ ] LinkedIn 帖子草稿
