...
- **界面是左右分屏，两边严格锁步**：同一个 seed、同一帧、同一根管道。
  谁先撞都算这一局结束，**两边同时重开并换一个新 seed**。等待期间两边都不动 ——
  AI 不能偷跑，一旦它比玩家多走几帧就不在同一根管道上了，左右对照立刻失效。
  **每局必须换 seed**：AI 是确定性的（eval_epsilon=0、无 Dropout/BatchNorm），
  ...
  锁步的代价：AI 的**当前分数必然等于**玩家的（同一组管道、鸟的 x 也相同），
  所以"谁分高"不再是有意义的对照，差距改由 `outlasted you`（你先掉下去的局数）
  和讲解区里的 78.2 / 6.8 承担。
