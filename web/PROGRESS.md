...
- 2026-09-07（云端 routine，B1 收尾）：**补上 `web/tools/stall_check.mjs`** ——
  上一轮（`a0ad404`）把断流修好了，但 `web_plan.md` §7.1 要求的回归脚本没写。
  修好的代码没有守卫脚本，等于把这个 bug 交给下一轮的运气。这一轮只做这件事，
  **没有改任何页面代码**（`git status` 里只有新增的那一个文件）。

  **判据选得和机器速度无关**，这是这个脚本的关键。上一条日志已经记了教训：
  CDP 的 `setCPUThrottlingRate` 不节流 worker，headless 里 worker 永远全速，
  所以"AI 有没有飞得够远"这种判据在快机器上是废的。这里改成盯两个量：
  玩家死亡那一刻的 `plan.length = P0`，之后 **plan 必须继续涨、`aiFrame`
  必须越过 P0**。断流的时候这两个量当场冻住，与机器多快无关。

  **负向对照（本轮实测，不是推演）**：按断流版本改了一份临时页
  （续命条件加回 `youAlive`、`endRound()` 里加回预取），跑同一个脚本：

  | | plan（玩家死后） | aiFrame | AI 分数 | stalls | 退出码 |
  |---|---|---|---|---|---|
  | 断流版（临时对照页） | 176 -> **176，涨了 0 次** | 停在 176 | **5** | **+5170** | **1** |
  | 当前 `index.html` | 192 -> **1520，涨了 26 次** | 1105（越过 P0 913 帧） | **35** | +0 | 0 |

  对照页跑完就删了，没有提交。**这条检查确实抓得住这个 bug** —— 不是
  "写了个脚本它过了"，是"把 bug 放回去它会红"。单次运行约 50 秒。

  **两件没做到的事，别当成做到了**：
  - 计划里写的"玩家飞到第 5 根再主动撞死"没实现。页面只暴露
    `window.__stats()`，外部脚本读不到鸟的高度，30 fps 下靠 CDP 回打按键
    有几十毫秒抖动，飞多远纯看运气（实测两次都死在第 0 根）。
    但**玩家死在第几根与这个 bug 无关** —— 断流由"玩家死"这个事件触发，
    不由分数触发，对照表里 5 根 vs 35 根的差距已经说明判据是有效的。
    玩家分数只打印、不作判据，别为了凑措辞把判据写成一个随机量。
  - **真机仍未验证。** 这个 bug 本来就是真机报的、headless 全绿放过去的。
    脚本现在能抓住"断流"这个具体机制，但抓不住"worker 产出速率跟不上"
    这一类只在真机上出现的问题。真机走查仍然要人做（留给 B6 的清单）。

  **给下一轮的环境备注**（省得再查一遍）：这个容器里 Chromium 不在
  `/usr/bin`，在 `/opt/pw-browsers/chromium-<版本>/chrome-linux/chrome`。
  `stall_check.mjs` 会自己按目录名扫（不写死版本号），但**其余几个
  `*_check.mjs` 的候选路径里没有这一条**，跑它们之前要先
...
- 2026-09-07（云端 routine，B2）：**管道不显示 —— 写了复现脚本，未能复现。**
  交付 `web/tools/pipe_render_check.mjs`（CDP 驱动）+ `web/tools/pipe_probe.js`
  （在页面上下文里跑的探针）。按 `web_plan.md` §2 的硬性要求，这一轮
  **没有改任何渲染代码** —— `git status` 里只有这两个新文件，
  `index.html` / `render.js` / `game.js` 一个字都没动。

  **结论：未能复现。** 三种状态里这是"未能复现"，不是"已修复"。

  ### 脚本分两段，各自证明什么

  **A 段·确定性逐帧对拍**（证明 `game.js` + `render.js` 这一对没问题）。
  在页面里 `import` 真正在跑的那两个模块，用离屏画布逐帧画，然后拿同一帧的
  `observeState()` 和画布像素**逐根管道**对拍。10 局固定 seed（20260907..16），
  **每一帧都判，不是抽帧**：

  | | 数字 |
  |---|---|
  | 帧数 | **8651** 帧（每局跑到小鸟撞死，分数 2~84） |
  | 判定样本 | **33810 根次**（另有 2538 根次因为刚从右边冒头、期望像素 < 300 没判） |
  | 最低覆盖率 | **1.0000**（不是"接近 1"，是逐帧最小值就等于 1） |
  | 幽灵像素帧 | **0** |

  覆盖率 = 这根管道该有管道色的像素里，实际是管道色的比例。整根没画出来 = 0。
  **全程 1.0000 意味着状态里有几根、画面上就一根不差地有几根。**

  **B 段·真页面连续性**（证明 index.html 那两块真画布上也没问题）。
  页面把 `you` / `aiGame` 关在模块作用域的 let 里，外部读不到状态，所以换了个
  **不需要状态**的判据：管道每帧恒定左移 5px，那么任何一根还在屏幕中间的竖条，
  下一次采样时必须仍然在、且正好挪了 5×Δ帧。管道"没显示"必然表现为竖条半路消失。
  AI 那块的 Δ帧从 `__stats().aiFrame` 精确读；玩家那块只在"玩家在玩 && AI 还活着"
  时判（这时两边才逐帧同步）。实测 **8474 帧采样、48114 根次判定、半路消失 0 次**。

  ### 三个负向对照（这是"没复现"这个结论唯一的可信度来源）

  没有它们，"全绿"只能证明脚本没在干活。三个都是**本轮实测**，不是推演：

  | 对照 | 做法 | 结果 |
  |---|---|---|
  | A-1 漏画一根 | 传给 `Renderer.draw()` 的快照里删掉中间那根（游戏状态不动） | 覆盖率 **0.000**，报红，退 1 |
  | A-2 挪偏一根 | 同一根的 x 加 30px | 覆盖率 **0.164** + 280 个幽灵帧，报红，退 1 |
  | B-1 真页面漏画 | 临时页把 `Renderer` 包一层、周期性少画一根（只影响页面自己的实例） | B 段报 **20 次半路消失**，退 1；**A 段仍是绿的**，说明两段确实各测各的 |

  B-1 用的两个临时文件（`__b2_neg.html` / `__b2_neg_render.js`）跑完就删了，没有提交。

  ### 踩过的三个坑，全是**假阳性**，别在下一轮重新踩一遍

  这三个都曾让脚本"复现到了问题"，查下去全是判据自己写错了。教训已经写进
  `pipe_probe.js` 的注释，这里留个索引：

  1. **幽灵检测用错了掩码。** 第一版拿"颜色过滤过的掩码"去圈管道的合法地盘，
     几乎每帧都报几十个幽灵像素。真相：管道调色板 35 色里有 3 色和地面/小鸟
     撞色被判成歧义、在掩码里是 0，可它们画出来之后因为 x/y 是小数被双线性
     混成了**纯管道色**。**几何（alpha）和颜色是两件事，不能共用一张掩码。**
  2. **竖条判据 `col[X] >= 3`。** 每帧都多出一根纹丝不动的 12px 竖条在 x=75~86
     —— 那是**小鸟**：`playery` 是小数，鸟边缘和黑背景混出的暗色最近邻落到了
     管道的暗绿上。
  3. **改成 `col[X] >= 40` 还是不行。** 小鸟不再自成一根了，但它**压在管道上**
     时会把那两三列从 ~50 打到 40 以下，一根管道被劈成两段，照样报假阳性。
     最后改成**上下两条 31px 带子都得有管道色**：管道上半截必然顶到 y=0、
     下半截必然盖过地面线，而小鸟只有 24px 高，不可能同时占满两条带子。
     顺带也躲开了"管子外侧几列只有帽檐、计数忽高忽低"造成的边界抖动。

  另外记一条环境坑：`--user-data-dir` 固定名字的话，上一次没退干净的 Chromium
  占着目录会让新实例**静默不起 CDP**，表现成"浏览器起不来"。现在带 pid 了。

  ### 为了让玩家那块画布真的被跑到，试了三版驾驶方式

  这一段值得记，因为前两版都让"玩家那块画布"实际上没被测到：

  | 版本 | 玩家分数 | 为什么不行 |
  |---|---|---|
  | 每 400 ms 从 CDP 打一下空格 | 0, 0 | 太慢，鸟直接摔 |
  | 每 160 ms 打一下 | 0, 0 | 太快，鸟顶到天花板。扇一下 velY=-5、每帧 +0.5，净位移 -5N+0.25N(N-1)，N=21 才为零 —— 5 帧一次是纯上升 |
  | 每 660 ms 打一下（悬停节奏） | 0, 1 | 高度稳住了，但它不会瞄准缝 |
  | **按像素驾驶**（现在这版） | **16, 2** | 从玩家画布里读鸟的质心 y 和前方第一根管子的缝心，和 A 段的 `pilot()` 同款 bang-bang |

  这里也踩了一个坑：找缝时"从上往下第一段非管道"是错的 —— 歧义色在管身里
  连成整行，算出来的缝心在天花板上，控制器一路狂扇。改成**第一段长度 ≥ 60 行**
  的非管道才算缝。**这个驾驶器只驱动玩家那只鸟，AI 走的还是 worker 里那份推理，
  一个字没动。**

  ### 没做到 / 没覆盖的，别当成已经排除

  - **真机仍未验证。** 这里跑的是 Blink headless，不是 iOS 的 WebKit。
    留给 B6 的人工清单。
  - **访问不了线上的 Cloudflare 部署**，只在本地 `http.server` 上复现过。
  - **B 段只跑了 2 局**（约 140 秒）。用户说的是"玩了几次后"，如果怀疑是
    长时间/多局累积的问题，把 `--live` 调大再跑，脚本支持。
  - 以下条件**一个都没试过**，下一轮要接着查的话从这里开始，
    不用重写脚本，`--live` 那一段加条件即可：
    **切后台再切回**（rAF 停了再回来）、**不同 DPR / 视口**（页面上画布是
    CSS 缩放的，`image-rendering: pixelated` 在缩放下的行为没测）、
    **刷新 / 连点重开**、**慢网络**、**worker 起不来的退化路径**
    （`aiPlayer` 主线程同步推理那条，本轮两段跑的都是 worker=on）。
  - 也就是说：**"管道不显示"在 `game.js` 生成/回收 和 `render.js` 绘制
    这两处已经基本排除**（33810 根次逐帧对拍 + 48114 根次连续性判定），
    剩下的嫌疑集中在**合成/缩放层**和**上面那几个没试过的生命周期条件**。
