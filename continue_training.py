...
class StableDQNAgent:
    ...
    def update_epsilon(self, episode_count):
        ...
        # 🔧 修改: 移除观察期逻辑，直接从0.5开始衰减
        # 基础衰减
        progress = self.decision_step / self.epsilon_decay_steps
        base_epsilon = max(self.epsilon_min, 0.5 - progress * (0.5 - self.epsilon_min))
        ...
        if episode_count < 10000:
            ...
            base_epsilon = max(base_epsilon, 0.15)
        elif episode_count < 50000:
            # 30,000-50,000局：低探索率（至少8%）
            base_epsilon = max(base_epsilon, 0.08)
...
def main():
    ...
    try:
        ...
        # 🔧 配置预训练模型路径（根据需要修改）
        PRETRAINED_MODEL_PATH = "saved_networks/stable_checkpoint_bs128_97500.pth"  # 修改为实际的预训练模型路径
        ...
        while episode_count < MAX_EPISODES:
            ...
            if terminal:
                ...
                if episode_reward > max_score:
                    ...
                    agent.save_model(f"saved_networks/improved_best_no_obs_{episode_reward:.0f}.pth", metadata)
                ...
                if episode_count % 100 == 0:
                    ...
                    if time.time() - last_save_time > 1800:  # 每30分钟
                        ...
                        agent.save_model(f"saved_networks/improved_checkpoint_no_obs_{episode_count}.pth", checkpoint_metadata)
                        last_save_time = time.time()
