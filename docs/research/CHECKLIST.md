...
## 【2026-09-08 云端第 8 次冷启动，非阻塞，但有一条重要更正】"选错方向"22% 子集反转——不要拿它做纠偏依据

`web/tools/wrong_direction_counterfactual.mjs`（新增）给第 5 次冷启动测到
的"22% 关键状态选错方向"补了真反事实：分叉状态，第一步强制走 0/1，之后
用**网络自己的策略**（不是 chase_gap）续跑 H=20 决策比较死亡率。结果和
朴素预期相反——在"选错方向"子集里，强制纠正成 chase_gap 判定的"安全"
动作，死亡率从 0-2.5%（网络实际会选的那个）暴涨到 44.6-47.1%，两个分布
（训练/部署）独立复现，McNemar z=-7.02/-5.39，效应量巨大不是噪声。

详细方法、逻辑论证（为什么能推断是"续跑策略不匹配"而不是"动作本身
危险"）、局限，见 `LAYER0-RESULTS.md` 第 8 次冷启动一节和本文件下面
"新-2 下方"关于第五次冷启动那条的更新框。**唯一需要本机/后续 session
记住的行动指令**：这 22% 子集不是网络的真实错误，**不要**用它做任何
"往 chase_gap 的判断上拉"的纠偏/模仿学习式改动——那类改动在这批数据上
是有害的，不是保守的小修小补。

距上次云端 session（`6f7ebd8`）无新提交、无新 checkpoint，`gap80_s0`
仍未产出，硬前置没变。

## 【2026-09-08 云端第 7 次冷启动，非阻塞】仍在等 `gap80_s0`，云端连续 3 次纯复核

距上次云端 session（`b0424f1`）约 4 小时，无新提交、无新 checkpoint，
未跑新实验，详见 `LAYER0-RESULTS.md` 对应小节。**建议把云端定时任务的
排期间隔从小时级拉长到天级**，直到本机产出下面这条 `gap80_s0` 训练或
新的诊断种子——否则后续冷启动大概率继续是空转复核，浪费预算。

## 【2026-09-08 云端第 6 次冷启动，非阻塞】死前 Q 轨迹部署分布版本已补完——云端这条线上已知诊断实验全部做完

`death_attribution_qtrace_deploy.mjs` / `_control_deploy.mjs`（结果见
`LAYER0-RESULTS.md` 对应小节）：部署分布上死前 maxQ 下沉 -3.711~-4.017，
和训练分布（-3.443）量级一致；对照组难通过/易通过下沉 +0.146/+0.202，
基本不下沉——**"死前 Q 下沉是死亡特异性信号"这条结论在部署分布上被独立
复现**。至此任务 A/B/C 在训练分布和部署分布上都至少做完一轮，云端目前
**没有已知的、在 `base_s0` 这个 checkpoint 上还没做过的诊断实验**。

硬前置仍然是本文件下面置顶的本机训练（`gap80_s0`）——`web/model/` 下
截至本次冷启动仍只有 `base_s0`/`s0_a`/`s0_b`，没有新 checkpoint。如果
下一轮云端定时任务冷启动时这两点仍未变化，建议把云端排期间隔进一步拉长。
...
> **⚠️ 2026-09-08 第 8 次冷启动更新：这个 22% 子集不代表网络"选错"，
> 不要拿它当优化目标或纠偏信号**——`web/tools/wrong_direction_counterfactual.mjs`
> 做了真反事实（分叉状态，第一步强制走 0/1，之后用网络自己策略续跑，
> 不是 chase_gap 续跑），发现在这个子集里强制网络改走 chase_gap 认为
> "安全"的动作，死亡率从 0-2.5% 暴涨到 44.6-47.1%（两个分布独立复现，
> `LAYER0-RESULTS.md` 第 8 次冷启动一节）。也就是说网络在这些分歧点上
> 的选择在它自己的策略框架下反而更优，"选错方向"这个标签是相对
> chase_gap 这一个特定策略定义的，不是相对客观最优。**如果以后想拿
> chase_gap 的判断给网络做模仿学习式纠偏（往它的选择上拉 reward/target），
> 这次数据是明确的反对证据**——单点纠正一个自洽策略的动作、不同时改变
> 它自己的后续策略，实测是有害的。这个 22% 数字本身还可以继续跟踪
> （比如看架构改动后是不是变化），但不能默认"变小=变好"，需要每次都
> 配一次同样的反事实检验才能下结论，不能只看比例本身。
